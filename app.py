...
st.set_page_config(page_title="Miuul DSMLBC11 Blackbox Grubu Streamlit Eğitimi")
...
st.sidebar.markdown("""[Anıl Şanlı - Kaggle](https://www.kaggle.com/sanlian)""")
...
if page == "Giriş":
    ...
    st.write(
        """Bu site Miuul DSMLBC11 Blackbox Grubu Streamlit Eğitimi için hazırlanmıştır.""")
    st.write(
    ...
    checkbox_value = st.checkbox("Hazır Mıyız?")
    if checkbox_value:
        slider_value = st.slider("Ne Kadar Hazırız?", 0, 100)
        if slider_value == 0:
            st.write("""Lütfen ne kadar hazır olduğunuzu seçiniz.""")
        elif slider_value < 85:
            st.write("""Yeteri kadar hazır değilsiniz.""")
            st.write("""Hazır olup tekrar deneyin.""")
        else:
            st.write("""HAYDİ BAŞLAYALIM!""")
    ...
    st.markdown("<h3 style='text-align:left;'>Cinsiyet</h3>", unsafe_allow_html=True)
    grouped_by_gender = df.groupby('gender').count().sort_values(by='Contract')
    grouped_by_gender = grouped_by_gender["Contract"]
    st.bar_chart(grouped_by_gender, use_container_width=True)
    ...
    st.markdown("<h1 style='text-align:center;'>Miuul DSMLBC11 Blackbox Grubu Eğitimi</h1>", unsafe_allow_html=True)
